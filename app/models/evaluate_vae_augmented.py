...
model.load_state_dict(torch.load('vae_augmented_cnn.pth', map_location=device))
model.eval()
...
with open(results_path, 'a', newline='') as f:
    ...
    for cls in target_names + ['macro avg']:
        row = report_dict[cls]
        writer.writerow(['vae_augmented', cls.replace(' ', '_'), round(row['precision'], 4), round(row['recall'], 4), round(row['f1-score'], 4)])

print('vae_augmented results appended to results/results.csv')