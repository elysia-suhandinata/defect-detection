...
with open(results_path, 'a', newline='') as f:
    writer = csv.writer(f)
    if not file_exists:
        writer.writerow(['method', 'class', 'precision', 'recall', 'f1'])
    for cls in target_names + ['macro avg']:
        row = report_dict[cls]
        writer.writerow(['baseline', cls.replace(' ', '_'), round(row['precision'], 4), round(row['recall'], 4), round(row['f1-score'], 4)])

print('baseline results appended to results/results.csv')